...
__global__ void render(float3 *d_out, int w, int h, float t) {
  ...
  glm::vec3 light{0.0f, 2.0f, 90.0f};
  ...
  while (STEPS --> 0)
  {
    ...
    glm::vec3 ball = glm::vec3(0.0f, -10.0f - sin(t / 5.0f) * 2.0f, 80.0f);
    if (glm::distance(ray.pos, ball) < 10.0f)
    {
      ray.dir = glm::reflect(ray.dir, glm::normalize(ray.pos - ball));
      ray.dir = glm::normalize(ray.dir);
      d_out[i].x += glm::dot(ray.dir, glm::normalize(ray.pos - light));
      ray.pos += ray.dir;
    }
    if (ray.pos.y > 2.0f)
    {
      d_out[i].z = sin(ray.pos.x) * 2.0f + cos(ray.pos.z) * 2.0f;
